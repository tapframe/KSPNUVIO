...
class KSPlayerView: UIView {
    ...
    private func setupPlayerCallbacks() {
        ...
        // Subtitle defaults; React props will override via applySubtitleAppearance()
    ...
    private func applySubtitleAppearance() {
        ...
        // Position
        playerView?.setSubtitleBottomOffset(CGFloat(truncating: subtitleBottomOffset))
    ...
    private func parseColor(_ raw: String) -> UIColor? {
        ...
        if s.hasPrefix("#") {
            ...
            if hex.count == 6 {
                let r = CGFloat(Int(hex.prefix(2), radix: 16) ?? 255) / 255.0
                let g = CGFloat(Int(hex.dropFirst(2).prefix(2), radix: 16) ?? 255) / 255.0
                let b = CGFloat(Int(hex.dropFirst(4).prefix(2), radix: 16) ?? 255) / 255.0
                return UIColor(red: r, green: g, blue: b, alpha: 1.0)
            }
            if hex.count == 8 {
                let a = CGFloat(Int(hex.prefix(2), radix: 16) ?? 255) / 255.0
                let r = CGFloat(Int(hex.dropFirst(2).prefix(2), radix: 16) ?? 255) / 255.0
                let g = CGFloat(Int(hex.dropFirst(4).prefix(2), radix: 16) ?? 255) / 255.0
                let b = CGFloat(Int(hex.dropFirst(6).prefix(2), radix: 16) ?? 255) / 255.0
                return UIColor(red: r, green: g, blue: b, alpha: a)
            }
            return nil
        }

        func parseCSV(_ inner: String) -> [CGFloat] {
            inner.split(separator: ",").map { part in
                CGFloat(Double(part.trimmingCharacters(in: .whitespacesAndNewlines)) ?? 0)
            }
        }

        if s.hasPrefix("rgba("), s.hasSuffix(")") {
            let inner = String(s.dropFirst(5).dropLast())
            let v = parseCSV(inner)
            guard v.count == 4 else { return nil }
            return UIColor(red: v[0] / 255.0, green: v[1] / 255.0, blue: v[2] / 255.0, alpha: min(max(v[3], 0), 1))
        }
        if s.hasPrefix("rgb("), s.hasSuffix(")") {
            let inner = String(s.dropFirst(4).dropLast())
            let v = parseCSV(inner)
            guard v.count == 3 else { return nil }
            return UIColor(red: v[0] / 255.0, green: v[1] / 255.0, blue: v[2] / 255.0, alpha: 1.0)
        }
